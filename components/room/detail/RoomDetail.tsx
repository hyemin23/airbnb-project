...
const Container = styled.div`
  ...
  .room-detail-infos {
    ...
    .room-detail-description {
      /* 모든 공백을 보여줌 한 줄이 너무 길어지면 자동으로 줄 바꿈 */
          ...
          {!isEmpty(room.bedList) && (
            <>
              ...
              <ul className="room-detail-bed-type-list">
                ...
                {room.publicBedList.map((bedroom: any, index: number) => (
          ...
          {!isEmpty(room.amenities) && (
            <>
              ...
              <ul className="room-detatil-conveniences-list">
                {room.amenities.map((amenities: string, index: any) => {
                  return (
                    <li key={index}>
                      <RoomAmentityIcon amenities={amenities} />
                      {amenities}
                    </li>
                  );
                })}

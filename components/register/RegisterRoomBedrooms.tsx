...
import RegisterRoomBedTypes from "../room/register/RegisterRoomBedTypes";
...
const RegisterRoomBedrooms: React.FC = () => {
  ...
  const bedroomCount = useSelector((state) => state.registerRoom.bedCount);
  ...
  const onChangeMaximumGuestCount = (value: number) => {
    dispatch(registerRoomAction.setMaximumGuestCount(value));
  };

  // * 침실 개수 변경 시
  const onChangeBedroomCount = (event: React.ChangeEvent<HTMLSelectElement>) => {
    dispatch(registerRoomAction.setBedroomCount(getNumber(event.target.value) || 0));
  };

  // * 침대 개수 변경 시
  const onChangeBedCount = (value: number) => {
    dispatch(registerRoomAction.setBedCount(value));
  };
  return (
    <Container>
      ...
      <p className="register-room-bed-type-info">
        각 침실에 놓인 침대 유형을명시하면 숙소에 침대가 어떻게 구비되어 있는지 게스트가 잘 파악할 수 있습니다.
      </p>
